...
int sigaddset(sigset_t *set, int signo) {
    if(set == NULL || (signo >= 32 || signo <= 0)){
        errno = EINVAL;
        return -1;
    }
    set->val[0] |= 1UL << (signo - 1);
    return 0;
}
...
int sigaction(int signo, struct sigaction *nact, struct sigaction *oact) {
    nact->sa_flags |= SA_RESTORER;
    nact->sa_restorer = (void (*)(void))&sys_rt_sigreturn;
    long ret = sys_rt_sigaction(signo, nact, oact, sizeof(sigset_t));
    WRAPPER_RETval(int);
}

sighandler_t signal(int signo, sighandler_t handler) {
    if((signo >= 32 || signo <= 0) || handler == SIG_ERR){
        errno = EINVAL;
        return SIG_ERR;
    }
